...
/*https://iot-tool.obs-website.cn-north-4.myhuaweicloud.com/
  解析网站
*/
#define ClientId "68559884d582f200183363d9_3333_0_1_2025070917"
#define Username "68559884d582f200183363d9_3333"
#define Password "a473e64e939dd45eee9cb72d817a2ff8c12aa9a2138f750d70eff49756546534"

#define MQTT_DEVICES_PWD ""

#define HOST_ADDR "117.78.5.125"

#define DEVICE_ID "68559884d582f200183363d9_3333"
...
void send_msg_to_mqtt(e_iot_data *iot_data) {
  ...
  if (root != NULL) {
    ...
    //空气质量
    sprintf(str, "%5.2fppm", iot_data->ppm);
    cJSON_AddStringToObject(pro_obj, "ppm", str);
    // 电机状态
    ...
    // nfc状态
    if (iot_data->nfc_state == true) {
      cJSON_AddStringToObject(pro_obj, "nfcStatus", "ON");
    } else {
      cJSON_AddStringToObject(pro_obj, "nfcStatus", "OFF");
    }
...
void set_nfc_state(cJSON *root) {
  ...
  if (status_obj != NULL) {
    value = cJSON_GetStringValue(status_obj);
    printf("[NFC_CONTROL] value = %s\n", value);
    if (!strcmp(value, "ON")) {
      event.data.iot_data = IOT_CMD_NFC_ON;
    } else if (!strcmp(value, "OFF")) {
      event.data.iot_data = IOT_CMD_NFC_OFF;
    }
    smart_home_event_send(&event);
  }
}
...
void mqtt_message_arrived(MessageData *data) {
  ...
  //比原数据多一位的原因是：printf、strlen 或拼接Topic）会 越界读取内存，直到遇到随机内存中的\0，多出来的这一位用来放\0
  MQTTMessage message;
  ...
  printf("rsptopic = %s\n", rsptopic);
  // printf("cbf8b47c-f1d2-4ed3-9b9e-8b8f309bd229");
  // response message
  message.qos = 0;
  message.retained = 0;
  message.payload = payload;
  ...
  if (root != NULL) {
    ...
    if (cmd_name != NULL) {
      ...
      } else if (!strcmp(cmd_name_str, "nfc_control")) {
        set_nfc_state(root);
        printf(">>> Entered set_nfc_state()\n");
