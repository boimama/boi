...
void nfc_menu_entry(lcd_menu_t *menu);
...
/* 所有的菜单集合数组,方便遍历查询*/
lcd_menu_t *lcd_menus[] = {&light_menu,&fan_menu,&nfc_menu};
/* 菜单的个数*/
...
void nfc_menu_entry(lcd_menu_t *menu)
{
   int nfc_state = get_nfc_state();
   if(nfc_state)
   {
       nfc_set_state(false);
       lcd_set_nfc_state(false);
   }else{
       nfc_set_state(true);
       lcd_set_nfc_state(true);
   }
}
...
uint8_t nfc_read_buf[128] = {0};
uint8_t eep_read_buf[128] = {0};
void SavePassword(unsigned char *nfcData) {
    eeprom_readbyte(0xFF,&password_count);
    if (password_count >= MAX_PASSWORDS) {
        printf("密码存储已满！\n");
        return;
    }
    uint16_t eeprom_index = password_count * PASSWORD_LENGTH;
    int ret = eeprom_write(eeprom_index, nfcData, PASSWORD_LENGTH);
    if (ret != PASSWORD_LENGTH) {
        printf("EEPROM 写入失败，返回 %d 字节\n", ret);
        return;
    }
    printf("保存第 %d 个密码: %s\n", password_count + 1, nfcData);
    password_count++;
    eeprom_writebyte(0xFF,password_count);
}
...
void smart_home_su03t_cmd_process(int su03t_cmd)
{
    switch (su03t_cmd)
    {
        ...
        case temperature_get:
        {
            ...
            su03t_send_double_msg(1, temp-7);
        }
...
{
    sprintf(ppm_db.text.name, "%.01fppm", ppm);
}
...
{
    sprintf(temp_db.text.name, "%.01f℃ ", temperature-7);
    /* 对温度做高温和正常的区分*/
    if(temperature > 35)
    {
...
void lcd_set_nfc_state(bool state)
{
    strcpy(nfc_menu.text.name,state? "NFC开" :"NFC关");
    nfc_menu.img.img = state? img_nfc_on : img_nfc_off;
}
